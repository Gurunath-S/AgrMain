...
app.post("/api/setup/test-connection", async (req, res) => {
  ...
  const testUrl = `mysql://${user}:${encodeURIComponent(password)}@${host}:${port}/information_schema`;
  const prisma = new PrismaClient({
    datasources: {
      db: { url: testUrl }
    }
...
app.post("/api/setup/save-config", async (req, res) => {
  ...
  const rootUrl = `mysql://${user}:${encodeURIComponent(password)}@${host}:${port}/information_schema`;
  ...
  try {
    console.log(`[Setup] Checking/Creating database: ${database}`);
    await rootPrisma.$executeRawUnsafe(`CREATE DATABASE IF NOT EXISTS \`${database}\``);
  } catch (err) {
    console.error("[Setup] Database creation failed:", err);
    return res.status(500).json({ success: false, error: "Failed to create database: " + err.message });
  } finally {
    await rootPrisma.$disconnect();
  }

  const newDbUrl = `mysql://${user}:${encodeURIComponent(password)}@${host}:${port}/${database}`;
  ...
  const migrationCmd = `node "${prismaCliPath}" db push --schema="${schemaPath}" --accept-data-loss`;

  console.log(`[Setup] Executing database push: ${migrationCmd}`);
  ...
  exec(migrationCmd, {
    ...
    if (error) {
      console.error("[Setup] Prisma push error:", stderr || error.message);
      return res.status(500).json({ success: false, error: "Failed to apply migrations: " + (stderr || error.message) });
    }
    console.log("[Setup] Prisma push output:", stdout);
